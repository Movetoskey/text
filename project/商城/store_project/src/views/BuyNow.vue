<template>
	<div class="buyNow">
		...
		<div class="main">
			...
			<div class="goodsList">
				<van-card v-for="orderList in settlementGood"
				  :num="orderList.num"
			...
			<div class="totalPrice">
				<van-cell-group>
				  <van-cell title-style="text-align:right;">
					  <template #default>
							<span>小计：<em>{{(settlementGood[0].num*settlementGood[0].oneprice) | money}}</em></span>
					  </template>
					  <template #title>
							<span>共<em>{{settlementGood[0].num}}</em>件商品</span>
					  </template>
				  </van-cell>
				</van-cell-group>
				<van-row>
				...
				<van-submit-bar button-text="提交订单" @submit="onSubmit">
					<template #default>
						<span class="submitPrice">合计：<em>{{(settlementGood[0].num*settlementGood[0].oneprice) | money}}</em></span>
					</template>
				</van-submit-bar>
			</div>
		</div>
		...
		<van-action-sheet v-model="show" title="付款详情">
			<div class="content">
				<van-cell-group>
				  ...
				  <van-cell title="需付款" :value="(settlementGood[0].num*settlementGood[0].oneprice) | money" />
				</van-cell-group>
...
<script>
	...
	export default{
		...
		data(){
			return{
				settlementGood: this.$store.state.selectGoods,
				show: false,
				showList:false,
				chosenCoupon: -1,
				coupons: [coupon],
		...
		methods:{
			...
			pay(){
				Toast('付款成功')
				this.settlementGood.forEach((item,index)=>{
