<template>
  <div class="paying">
    <ul v-for="(val, id) in times" :key="id">
      <li>
        <h1 class="date">
        ...
        <div>
          ...
          <p>
            <van-button plain type="primary" size="small" @click="Cancel(id)"
              >取消订单</van-button
...
export default {
  ...
  methods: {
    Cancel(index) {
      this.times[index].text = "已取消";
      this.$set(this.newArray, this.newArray.length, {
        time: this.times[index].time,
        items: this.times[index].items,
        text: "已取消",
      });
      location.reload();
